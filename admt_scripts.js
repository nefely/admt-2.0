...
admt_start = () => {
	...
	$(document).ready(function(){
	    ...
	    window.params_object = params_object
	    ...
	    params_object.intst = getURLParameter(window.location.href, 'intst') ? getURLParameter(window.location.href, 'intst') : "unkwn"
	    params_object.em = getURLParameter(window.location.href, 'em') ? getURLParameter(window.location.href, 'em') : "unkwn"
	    params_object.ph = getURLParameter(window.location.href, 'ph') ? getURLParameter(window.location.href, 'ph') : "unkwn"
	    params_object.n = getURLParameter(window.location.href, 'n') ? getURLParameter(window.location.href, 'n') : "unkwn"
	    params_object.nk = getURLParameter(window.location.href, 'nk') ? getURLParameter(window.location.href, 'nk') : "unkwn"
	    params_object.fy = getURLParameter(window.location.href, 'fy') ? getURLParameter(window.location.href, 'fy') : "unkwn"
	    params_object.psx = getURLParameter(window.location.href, 'psx') ? getURLParameter(window.location.href, 'psx') : "unkwn"
	    params_object.fn = getURLParameter(window.location.href, 'fn') ? getURLParameter(window.location.href, 'fn') : "unkwn"
	    params_object.ps = getURLParameter(window.location.href, 'ps') ? getURLParameter(window.location.href, 'ps') : "unkwn"
	    params_object.of = getURLParameter(window.location.href, 'of') ? getURLParameter(window.location.href, 'of') : "unkwn"
	    params_object.bd = getURLParameter(window.location.href, 'bd') ? getURLParameter(window.location.href, 'bd') : "unkwn"

	    params_object.ep1 = getURLParameter(window.location.href, 'ep1') ? getURLParameter(window.location.href, 'ep1') : "unkwn"
	    params_object.ep2 = getURLParameter(window.location.href, 'ep2') ? getURLParameter(window.location.href, 'ep2') : "unkwn"
	    params_object.ep3 = getURLParameter(window.location.href, 'ep3') ? getURLParameter(window.location.href, 'ep3') : "unkwn"

	    params_object.frq = getURLParameter(window.location.href, 'frq') ? getURLParameter(window.location.href, 'frq') : 0
	    params_object.flw = getURLParameter(window.location.href, 'flw') ? getURLParameter(window.location.href, 'flw') : ""

	    // rewrire this params if need
	        ...
	        // params_object.web_push_notification_setting.actionMessage = "Hi, handsome 😍 I'm free right now, and I'd like to have some fun. Want to keep me company?"
	    ...
	    window.AssignLinkValueToLink = (obj) => {
	        for (const [key , value] of Object.entries(obj)) {
	            $(`.${key.replaceAll("_" , "-") + "-go"}`).attr("href" , value)
	        }
	    }

	    window.formFinalLinkObject = () => {
	        setTimeout(()=>{
	            // general
	            __aid_sorc = params_object.aid ? `aid_sorc=${params_object.aid}` : `aid_sorc=`
	            ...
	            __a = params_object.a ? `a=${params_object.a}` : `a=`
	            __sx = params_object.sx ? `sx=${params_object.sx}` : `sx=`
	            __intst = params_object.intst ? `intst=${params_object.intst}` : `intst=`
	            __em = params_object.em ? `em=${params_object.em}` : `em=`
	            __ph = params_object.ph ? `ph=${params_object.ph}` : `ph=`
	            __n = params_object.n ? `n=${params_object.n}` : `n=`
	            __nk = params_object.nk ? `nk=${params_object.nk}` : `nk=`
	            __fy = params_object.fy ? `fy=${params_object.fy}` : `fy=`
	            __psx = params_object.psx ? `psx=${params_object.psx}` : `psx=`
	            ...
	            __bd = params_object.bd ? `bd=${params_object.bd}` : `bd=`

	            __ep1 = params_object.ep1 ? `ep1=${params_object.ep1}` : `ep1=`
	            __ep2 = params_object.ep2 ? `ep2=${params_object.ep2}` : `ep2=`
	            __ep3 = params_object.ep3 ? `ep3=${params_object.ep3}` : `ep3=`

	            __frq = params_object.frq ? `frq=${params_object.frq}` : `frq=`
	            __flw = params_object.flw ? `flw=${params_object.flw}` : `flw=`
	    ...
	    // web push
	    // params_object.wpn_id = getURLParameter(window.location.href, 'wpn_id')

	    // watch
	    window.params_object = new Proxy(params_object, {
	...
	if ( admt_array.filter(item => item.type == "in_page_push" && item.active == true)[0] ) {
		$(document).ready(function(){
			...
			in_page_push_get_random_object = () => {
				...
				in_page_push_array_of_showed_object.push(in_page_push_number_woman)
	...
	if ( admt_array.filter(item => item.type == "exit_popup" && item.active == true)[0] ) {
		$(document).ready(function(){
			...
			comebacker_get_random_object = () => {
				...
				comebacker_array_of_showed_object.push(comebacker_number_woman)
			...
			window.comebacker_mouse_location
			window.comebacker_window_location
			
			onmousemove = function(e){
				comebacker_mouse_location = {
					"x": e.clientX,
					"y": e.clientY
				}
				comebacker_window_location = {
					"x_start": 1, 
					"x_end": window.innerWidth - 1, 
					"y_start": 1, 
					"y_end": window.innerHeight - 1, 
				}
			}
	...
	if ( admt_array.filter(item => item.type == "second_offer" && item.active == true)[0] ) {
		$(document).ready(function(){
			document.querySelector("body").insertAdjacentHTML("beforeend", 
		  	`<a class="second-offer-go" style="display: none !important; padding: 0: !important; margin: 0 !important; visibility: hidden !important; opacity: 0 !important;"></a>`
			);
			...
			$("body").on("click", ".go", function() {
				...
				if (!admt_conf.collect_disabled) {
					...
					// core api
					if (admt_conf.collect_via == undefined || admt_conf.collect_via == "all" || admt_conf.collect_via == "api" ) {
						setTimeout(()=>{
							let core_api_data = {
								...
								birthdate: params_object?.bd ,
								phone: params_object?.ph ,
								source: admt_conf?.sorc_id ,
								landing: admt_conf?.landing_id ,
								vertical: admt_conf?.vertical ,
								gender: params_object?.sx ,
								username: params_object?.nk,
								age: params_object?.fy ,
								prefer_gender: params_object?.psx ,
								aff_cid: admt_conf?.subid ,
								aff_id: `${admt_conf?.campaign_id}_${admt_conf?.stream_id}` ,
								ip: params_object?.ip , 
							}
							for (let key in core_api_data) {
							    if (core_api_data[key] === "" || core_api_data[key] === undefined || core_api_data[key] == 'unkwn' || core_api_data[key] == null) {
							        delete core_api_data[key];
							    }
							}
							if (params_object.em !== "" && /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,10})+$/.test(params_object.em)) {
								$.ajax({
								    url: 'https://core-prod.orhdev.com/api/leads',
								    type: 'POST',
								    data: JSON.stringify(core_api_data),
								    headers: {
								        'Consumer-Api-Token': '299aff8598497c721c12a1a06009a2bf',
					...
					// old api 
					if (admt_conf.collect_via == "bd" || admt_conf.collect_via == "all" || admt_conf.collect_via == undefined) {
						setTimeout(()=>{
							let api_send_data = {
								name: params_object?.n ,
								last_name: params_object?.fn ,
	...
	// back button
	if ( admt_array.filter(item => item.type == "back_button" && item.active == true)[0] ) {
		$(document).ready(function(){
			document.querySelector("body").insertAdjacentHTML("beforeend", 
			`<a class="back-button-go" style="display: none !important; padding: 0: !important; margin: 0 !important; visibility: hidden !important; opacity: 0 !important;"></a>`
			);


			var back_button_array = [
				{
					id: "11111",
				},
				{
			...
			if ( getURLParameter(window.location.href, 'bb_id') && back_button_array.filter((item) => item.id == getURLParameter(window.location.href, 'bb_id'))?.[0]?.id ){
				...
				var back_button_number = Math.floor(Math.random() * back_button_array.length)
			...
			if (window.history && window.history.pushState) {
				...
				$(window).on('popstate', function() {
					...
					setTimeout(()=>{
						window.location.href = $(".back-button-go").attr("href")
					}, 500)
				});
			}
		});
						...
						prompts: [
							{
								...
								autoPrompt: true,
								icon: wpn_object_local.img,
								text: {
									/* limited to 90 characters */
									actionMessage: wpn_object_local.text,
									/* acceptButton limited to 15 characters */
									acceptButton: wpn_object_local.btn_yes,
									/* cancelButton limited to 15 characters */
									cancelButton: wpn_object_local.btn_no,
								},
								delay: {
									pageViews: 1,
									timeDelay: 10
